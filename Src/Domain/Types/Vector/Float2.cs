...
namespace CXUtils.Domain.Types
{
	...
	public readonly struct Float2 : IVectorFloat<Float2>
	{
		...
		/// </summary>
		public Float2 Rotate(float angle) => new Float2(
			x * MathUtils.Cos(angle) - y * MathUtils.Sin(angle),
			x * MathUtils.Sin(angle) + y * MathUtils.Cos(angle)
		);
		...
		/// </summary>
		public static Float2 FromAngle(float angle) =>
			new Float2(MathUtils.Cos(angle), MathUtils.Sin(angle));
		
		/// <summary>
		///		Calculates a Float2 using the specified <paramref name="angle"/> scaled with <paramref name="length"/>
		/// </summary>
		public static Float2 FromAngle(float angle, float length) =>
			new Float2(MathUtils.Cos(angle) * length, MathUtils.Sin(angle) * length);
