...
namespace CXUtils.Domain.Types
{
	...
	public readonly struct Float3x3 : IEquatable<Float3x3>, IFormattable
	{
		...
		public static Float3x3 Zero => new Float3x3(Float3.Zero, Float3.Zero, Float3.Zero);
		...
		public static Float3x3 operator +(Float3x3 a, Float3x3 b) => new Float3x3(a.iHat + b.iHat, a.jHat + b.jHat, a.kHat + b.kHat);
		public static Float3x3 operator -(Float3x3 a, Float3x3 b) => new Float3x3(a.iHat - b.iHat, a.jHat - b.jHat, a.kHat - b.kHat);

		public static Float3x3 operator *(Float3x3 matrix, float scalar) => new Float3x3(matrix.iHat * scalar, matrix.jHat * scalar, matrix.kHat * scalar);
		...
		public static Float3 operator *(Float3 vector, Float3x3 matrix) => new Float3(vector.x * matrix.iHat.x + vector.y * matrix.iHat.y + vector.z * matrix.iHat.z, vector.x * matrix.jHat.x + vector.y * matrix.jHat.y + vector.z * matrix.jHat.z, vector.x + matrix.kHat.x + vector.y * matrix.kHat.y + vector.z * matrix.kHat.z);

		public static Float3 operator *(Float3x3 matrix, Float3 matrix3x1) => new Float3(matrix.iHat.x * matrix3x1.x + matrix.jHat.x * matrix3x1.y + matrix.kHat.x * matrix3x1.z, matrix.iHat.y * matrix3x1.x + matrix.jHat.y * matrix3x1.y + matrix.kHat.y * matrix3x1.z, matrix.iHat.z * matrix3x1.x + matrix.jHat.z * matrix3x1.y + matrix.kHat.z * matrix3x1.z);
		...
		public static Float3x3 operator -(Float3x3 matrix) => new Float3x3(-matrix.iHat, -matrix.jHat, -matrix.kHat);
		...
		public static Float3x3 BuildDiagonal(Float3 value) => new Float3x3(new Float3(value.x), new Float3(0f, value.y), new Float3(0f, 0f, value.z));

		public static Float3x3 BuildScalar(float value) => new Float3x3(new Float3(value), new Float3(0f, value), new Float3(0f, 0f, value));
